...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            #hneeeeeeee
            crop_rate = 160
            #if framnum % crop_rate != 0:
            #cv2.rectangle(image, (int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1])), (255, 69, 0), bbox_thick)
            cv2.rectangle(image, (int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1])), (127, 255, 0), bbox_thick)
            ...
            if show_label:
                ...
                cv2.rectangle(image, (int(c1[0]), int(c1[1])), (int(c3[0]), int(c3[1])), (127,255,0), -1)  # filled
            ...
            if frame != None:
                ...
                if frame % crop_rate == 0:
                    cv2.putText(image, "Capture Now  {}".format(frame), (5, offset),
                                cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 2)

                else :
                    cv2.putText(image, "FPS {}".format(frame), (5, offset),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 2)
                    frame = 0
                offset += height_ratio
