import os
from main import display_video
...
def main(_argv):
    ...
    while True:
        ...
        if FLAGS.crop:
            ...
            if frame_num % crop_rate == 0:
                ...
                final_path = os.path.join(crop_path, 'frame_')
                try:
                    os.mkdir(final_path)
                except FileExistsError:
                    pass          
        ...
        if FLAGS.count:
            ...
            for key, value in counted_classes.items():
                print("Number of {}s: {}".format(key, frame_num))
            image = utils.draw_bbox(frame, pred_bbox, FLAGS.info, frame_num, allowed_classes=allowed_classes, read_plate=FLAGS.plate)
        else:
    ...
    return render_template('upload.html', filename=video_name)
def framnum() :
    return frame_num
